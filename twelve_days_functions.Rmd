...
output <- vector("character", length(gift))

for (i in seq_along(gift)){
  if (str_detect(gift[[i]], ("oose$"))|str_detect(gift[[i]], ("y$"))) {
    item <- gift[[i]] %>%
    str_replace("y$", "ies") %>% 
...
to create numbered days and then use those in your `pmalibralip_chr()`!
...
sing_line <- function(dataset, num, phrase_col){
  ...
  num_word <- dataset[num, "Day.in.Words"]
  ...
  # Step 2: Sing the gift phrases
    ...
    phrases <- dataset %>% 
      pull({{phrase_col}})
    if (num > 1) {
      phrases[c(1)] <- paste0("and ", phrases[c(1)])
    }
    phrases <- str_c(phrases[c(num:0)], collapse = "\n")
